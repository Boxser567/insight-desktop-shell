...
describe('Windows titlebar menu', () => {
  ...
  it('synchronizes the native controls with Harness light and dark themes', async () => {
    const main = await readFile('src/main/index.ts', 'utf8')
    const preload = await readFile('src/preload/windows-titlebar.ts', 'utf8')
    const harnessPreload = await readFile('src/preload/harness.ts', 'utf8')
    ...
    expect(main).toContain('applyWindowChromeTheme(mainWindow, isDark)')
    expect(main).toContain('applyNativeThemePreference(harnessThemePreference(dshHome))')
    expect(main).toContain('resolvedHarnessThemeDark = isDark')
