...
describe('GitHub release contract', () => {
  ...
  it('keeps trusted local plugin import alongside the bundled marketplace', async () => {
    ...
    expect(profile).not.toContain('patchBundledMarket')
    expect(profile).not.toContain('dshmarket')
    expect(profile).not.toContain('@changfenhuang/dsh-genui')
  ...
  it('preflights one complete release before starting native builds', async () => {
    ...
    expect(preflight).toContain('Enforce source intake policy')
    expect(preflight).toContain("git log --format='%an <%ae>|%cn <%ce>'")
    expect(preflight).toContain("grep -Eiq 'duzhimeng'")
