...
async function fixture() {
  ...
  for (const file of [input, metadata.entry, 'node_modules/pnpm/bin/pnpm.cjs', `node_modules/node/bin/${process.platform === 'win32' ? 'node.exe' : 'node'}`]) {
    ...
    await writeFile(join(root, file), file === input ? 'toggleSkill(name: string): void;' : '')
  }
...
it('rejects an old Runtime before replacing the development cache', async () => {
  ...
  await writeFile(join(root, input), 'setDraft(text: string): void;')
  await expect(inspectLocalRuntime(root)).rejects.toThrow('lacks the draft-safe Skill API')
})
