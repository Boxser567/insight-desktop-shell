...
export async function uploadV2Target(input) {
  ...
  for (const entry of names.sort((a, b) => a.name.localeCompare(b.name))) {
    ...
    if (current) {
      if (current.size !== bytes.length) throw new Error(`Existing Draft asset conflicts: ${assetName}`)
      const downloaded = await request(input, current.url, {
        headers: { Accept: 'application/octet-stream' }
      })
      const remote = Buffer.from(await downloaded.arrayBuffer())
      if (createHash('sha512').update(remote).digest('base64') !== digest) {
        throw new Error(`Existing Draft asset conflicts: ${assetName}`)
      }
      report.push({ name: assetName, sha512: digest, action: 'verified' })
      continue
    }
    ...
    report.push({ name: assetName, sha512: digest, action: 'uploaded' })
  }
  return report
}
