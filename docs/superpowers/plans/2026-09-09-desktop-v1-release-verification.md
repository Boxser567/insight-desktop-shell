...
2026-09-10 RC4/RC5 收口证据：产品负责人已在本机使用已安装的 `1.0.0-rc.3` 完成客户端内检查、下载、安装、重启至 `1.0.0-rc.4`，确认本次 ARM64 Candidate 更新链路通过。RC4 只发布 Apple Silicon 资产，其不可变目录和签名 Manifest 不再追加 Windows 文件。随后以提交 `ba3dc2c4b83e6cabbca56c3be3ca38bdcb584da3` 准备 `1.0.0-rc.5` 全平台 Candidate；[Release Run 34472407835](https://github.com/Boxser567/insight-desktop-shell/actions/runs/34472407835) 的预检、macOS ARM64、macOS Intel、Windows x64、Sonoma 和 Draft Job 全部成功，Draft 含两个 DMG、两个 ZIP、两个 ZIP blockmap、Windows EXE、EXE blockmap、两个 YAML、签名 Manifest 和签名文件共 12 项。[Stage Run 34475061375](https://github.com/Boxser567/insight-desktop-shell/actions/runs/34475061375) 通过 OIDC/STS 首次上传并回读校验 `desktop/releases/v1.0.0-rc.5/` 的 12 项对象，共 1,426,399,492 字节。[Promote Run 34475599657](https://github.com/Boxser567/insight-desktop-shell/actions/runs/34475599657) 随后成功发布 [v1.0.0-rc.5 Pre-release](https://github.com/Boxser567/insight-desktop-shell/releases/tag/v1.0.0-rc.5)，并将 OSS Candidate 权威指针从 `1.0.0-rc.4` 切换为 `1.0.0-rc.5`；公网 Candidate URL 返回 HTTP 200、版本 `1.0.0-rc.5` 和 60 秒缓存策略。Windows 安装包当前仍为明确接受的未签名 Candidate，需继续完成人工安装及后续更新验收。
...
2026-09-11 RC7 全平台暂存证据：RC6 已形成不可变的 ARM64-only Draft 和 OSS 版本目录，因此未向同版本追加 Intel/Windows 资产，而是以提交 `376198922dd481a8309794c1a98f414d3bf2b3b8` 递增准备 `1.0.0-rc.7` 并使用 `target=all`。本地 96 个测试文件共 606 项测试、TypeScript、Electron 构建及发布契约检查通过；[Release Run 34505540020](https://github.com/Boxser567/insight-desktop-shell/actions/runs/34505540020) 的预检、Apple Silicon、Intel、Windows x64、Sonoma 和签名 Draft 六个 Job 全部成功，两套 macOS DMG 均完成 Developer ID 签名、公证和 stapling，Sonoma 对最终 ARM64 DMG 的分发兼容验证通过。未公开 Draft 精确绑定上述提交并包含两个 DMG、两个 ZIP、两个 ZIP blockmap、Windows EXE、EXE blockmap、两个 YAML、签名 Manifest 和签名文件共 12 项。[Stage Run 34508573859](https://github.com/Boxser567/insight-desktop-shell/actions/runs/34508573859) 通过 GitHub OIDC、测试 Gateway 和目录级 STS 首次写入并回读校验 `desktop/releases/v1.0.0-rc.7/` 的 12 项对象，共 1,426,395,868 字节，报告确认 `scope=all`、`reusedImmutablePrefix=false`。Stage 完成后的中国大陆公网首次并发复验曾出现短暂 TLS 连接关闭；有限重试恢复后，Candidate 指针返回 HTTP 200、`1.0.0-rc.5` 和 60 秒缓存，RC7 Manifest、签名及两份 YAML 均返回 HTTP 200 和 31,536,000 秒 immutable 缓存；ARM64 DMG、Intel DMG、Windows EXE 的 `Range: bytes=0-0` 均返回 206，精确总大小依次为 256,121,706、261,741,333、267,898,253 字节。RC7 与 RC6 均未 Promote，RC5 仍是客户端可见 Candidate。Windows 安装包仍为明确接受的未签名 Candidate，RC7 Promote 前还需完成确切安装包验收。
