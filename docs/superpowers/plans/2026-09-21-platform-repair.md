...
- [x] Update profile generation to 8 and include generation 7 in migration selection.
- [x] Remove GenUI and Market by exact package identity regardless of dependency version; invalidate locks and install marker before modifying profile.
- [x] Exercise normalized dependency specs, missing package files, old generations, repeated invocation and unrelated plugin preservation.
- [x] Run `npx vitest run test/bundled-profile.test.ts`.
...
- [x] Add local reset, guarded by session revision; invalidate in-flight restore results. IPC only allows reset from the offline Shell main frame.
- [x] Connect Electron authentication partition cleanup and a retryable renderer action.
- [x] Add sanitized transport diagnostics, testing that secrets in error messages never reach logs.
- [x] Run focused authentication tests and `npm run typecheck`.
...
- [x] Add minimumSystemVersion 13.0 to build.mac, and Darwin 22.0.0 to macOS updater YAML.
- [x] Preserve and validate requirements when merging architecture metadata.
- [x] Verify updater metadata against the signed manifest before using its system requirement; return unsupported before activating an update.
- [x] Test mismatched metadata, insufficient OS, and unaffected supported OS.
