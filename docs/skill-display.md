...
专家技能菜单仅展示因赛 AI 内置技能：技能 ID 必须存在于内置展示表，且 Core 返回的实际生效路径必须以 `bundled-skills/<相同 ID>/SKILL.md` 结束（兼容 Windows 路径）。本机技能、无路径技能及覆盖内置技能的外部同名技能不进入此菜单，也不计入已选数量。不会为被覆盖的内置技能伪造一个可调用条目，不删除或禁用外部技能，也不改变 Core 的发现与调用规则。

该来源判断仅用于展示分类，不是文件完整性或安全认证。添加新的内置技能时需同步内置展示表及 `ui.json`，测试会检查其一致性。

菜单宽度 360px、总高度不超过 360px，并适应可用视口空间；搜索框高 32px，简介最多两行，悬停显示完整简介。搜索仍匹配完整文本。
