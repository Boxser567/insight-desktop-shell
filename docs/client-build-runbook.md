...
| macOS 签名时出现 `EMFILE: too many open files` | `@electron/osx-sign 1.3.3` 会通过 `Promise.all` 递归检查 App 内的文件；当 Runtime 和内置 Profile 合计超过万级文件时，runner 的软文件句柄限制会被耗尽 | 阶段 9；两个 macOS 构建 Shell 必须在 electron-builder 前将 `ulimit -n` 提升到 `65536`；`10240` 已在约 4.2 万文件的实际制品上证实不足，签名后仍执行完整 bundle 校验 |
