...
Profile、插件、工作区和会话都位于应用安装目录之外，升级不会覆盖它们。默认 Profile 随安装包提供必需的 Better Sidebar 和可卸载的 `dshmarket`；用户可通过 Plugin Market 管理社区插件，也可继续显式导入本地插件。启动失败时，恢复流程和 Safe Mode 只处理 Profile，不删除用户工作区或会话。
