...
function menuEntries(locale: 'en' | 'zh'): MenuEntry[] {
  const zh = locale === 'zh'
  return [
    { kind: 'command', command: 'show-about', label: zh ? '关于因赛AI' : 'About Insight AI' },
    { kind: 'separator' },
    ...
    { kind: 'command', command: 'check-for-updates', label: zh ? '检查更新' : 'Check for Updates' },
...
const menuStyles = `
  ...
  * { box-sizing: border-box; }
  ...
  .menuButton:focus-visible { outline:2px solid var(--insight-primary); outline-offset:-3px; }
